...
struct ItemCard: View {
    ...
    init(item: WaffliItem, onEdit: @escaping (WaffliItem) -> Void, onDelete: @escaping (WaffliItem) -> Void) {
        self.item     = item
        self.onEdit   = onEdit
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 16)
                .stroke(
                    justCompleted ? Color("Waffle").opacity(0.7) : Color("Waffle").opacity(0.3),
    ...
    private func triggerComplete() {
        justCompleted = true
        // Pequeño delay para que el usuario vea el flash antes de que desaparezca
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
            withAnimation(.easeInOut(duration: 0.4)) {
                item.completeAndArchive()
            }
        }
    }
}
...
struct EmptyStateView: View {
    var body: some View {
        VStack(spacing: 12) {
            ...
            Text("Nada aquí todavía")
                .font(.headline)
                .foregroundStyle(Color("Cocoa").opacity(0.7))

            Text("Toca + para crear tu nueva tarea")
                .font(.subheadline)
